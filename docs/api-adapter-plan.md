...
- Visible asset workspace QA aggregate diagnostics may expose root total, visible, hidden, and reconciliation markers proving compact rows plus hidden QA counts match safe local QA totals; they must remain diagnostics only and cannot become QA mutation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace QA readiness diagnostics may expose root pending/total count markers and a reconciliation marker proving the readiness state matches safe local QA totals across populated, no-QA, and unavailable workspaces; they must remain diagnostics only and cannot become QA mutation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace row QA count diagnostics may expose read-only row-level detail counts, pending detail counts, and reconciliation markers proving rendered safe QA details match that row's safe QA totals; they must remain diagnostics only and cannot become QA mutation, navigation, draft creation, publishing, sync, OAuth, credential, or commerce-write controls.
- Visible asset workspace row QA status diagnostics may expose read-only row-level status count rows and reconciliation markers derived from that row's safe QA previews; they must remain diagnostics only and cannot become QA mutation, navigation, draft creation, publishing, sync, OAuth, credential, or commerce-write controls.
- Visible asset workspace row QA readiness diagnostics may expose read-only row-level readiness state, pending/total counts, visible readiness summary copy, and reconciliation markers derived from that row's safe QA previews; they must remain diagnostics only and cannot become QA mutation, navigation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace row blocked capability diagnostics may expose read-only row-level blocked capability counts, key rows, and reconciliation markers derived from that row's safe blocked capability list; they must remain diagnostics only and cannot become navigation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace panel blocked capability diagnostics may expose read-only workspace-level blocked capability key rows and a reconciliation marker proving visible panel rows match the workspace blocked capability count; they must remain diagnostics only and cannot become claim editing, QA mutation, navigation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace WordPress draft readiness diagnostics may expose a read-only blocked reconciliation marker proving readiness ready counts do not exceed total local asset draft counts and total readiness counts match local draft count; they must remain diagnostics only and cannot become WordPress draft creation, page update, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace external-write diagnostics may expose read-only clamp reconciliation markers proving the root external-write state and visible external-write row both stay false; they must remain diagnostics only and cannot become external writes, WordPress draft creation, page update, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace availability diagnostics may expose a read-only visible availability row and reconciliation marker proving the visible row state matches the root workspace availability state across empty, ready, and unavailable panels; they must remain diagnostics only and cannot become external writes, WordPress draft creation, page update, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace draft count diagnostics may expose a read-only visible local candidate count row and reconciliation marker proving the visible count matches the root local asset draft count; they must remain diagnostics only and cannot become external writes, WordPress draft creation, page update, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace visible-row overflow diagnostics may expose read-only visible and hidden row count markers proving visible rows plus hidden overflow rows match the root local asset draft count; they must remain diagnostics only and cannot become external writes, WordPress draft creation, page update, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace type mix diagnostics may expose read-only per-type rows and reconciliation markers proving per-type local asset counts sum to the root local asset draft count; they must remain diagnostics only and cannot become external writes, WordPress draft creation, page update, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace local review-state diagnostics may expose read-only review-state rows and reconciliation markers derived from existing local asset preview state; they must remain diagnostics only, must not add product task statuses beyond `new`, `approved`, `rejected`, and `snoozed`, and cannot become task mutation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace source task diagnostics may expose read-only source task rows and reconciliation markers derived from local asset preview `sourceTaskId` values; they must remain diagnostics only and cannot become task mutation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace source task status diagnostics may expose read-only status rows and reconciliation markers derived from local asset preview `sourceTaskStatus` values; frontend adapters must clamp unknown or unsafe source statuses to Sprint 1 task statuses, and these diagnostics cannot become task mutation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace content block aggregate diagnostics may expose read-only root total, visible, hidden, and reconciliation markers proving compact rows plus hidden content block counts match safe local content block totals; they must remain diagnostics only and cannot become rich text editing, navigation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace content block type diagnostics may expose read-only workspace-level type rows and reconciliation markers derived from safe local content block previews; they must remain diagnostics only and cannot become rich text editing, navigation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace row content block type diagnostics may expose read-only row-level content block type count rows and reconciliation markers derived from that row's safe content block previews; they must remain diagnostics only and cannot become rich text editing, navigation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Visible asset workspace row content block count diagnostics may expose read-only row-level content block count and reconciliation markers proving row-visible type rows match that row's safe content block count; they must remain diagnostics only and cannot become rich text editing, navigation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
...
- Local asset editor content block type diagnostics may expose read-only type count rows and reconciliation markers derived from the selected asset's safe content block previews; they must remain diagnostics only and cannot become rich text editing, navigation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
- Local asset editor content block count diagnostics may expose read-only content block count and reconciliation markers proving editor-visible type rows match the selected asset's safe content block count; they must remain diagnostics only and cannot become rich text editing, navigation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
...
- Visible asset workspace row claim source diagnostics may expose read-only row-level source count rows and reconciliation markers derived from that row's safe claim previews; they must remain diagnostics only and cannot become claim editing, navigation, draft creation, publishing, sync, OAuth, credential, or commerce-write controls.
...
- Local asset editor QA readiness reconciliation diagnostics may expose read-only readiness pending/total counts and reconciliation markers proving editor readiness matches safe QA detail counts; they must remain diagnostics only and cannot become QA mutation, draft creation, publishing, sync, OAuth, credential, navigation, product edit, or commerce-write controls.
- Local asset editor QA status diagnostics may expose read-only status count rows and reconciliation markers derived from safe local QA previews; they must remain diagnostics only and cannot become QA mutation, draft creation, publishing, sync, OAuth, credential, navigation, product edit, or commerce-write controls.
- Local asset editor QA count diagnostics may expose read-only detail counts, pending detail counts, and reconciliation markers proving rendered safe QA detail rows match the selected asset's safe QA totals; they must remain diagnostics only and cannot become QA mutation, draft creation, publishing, sync, OAuth, credential, navigation, product edit, or commerce-write controls.
- Local asset editor source task context diagnostics may expose read-only source task id and safe source task status rows from the selected local asset preview; they must remain diagnostics only and cannot become task mutation, draft creation, publishing, sync, OAuth, credential, navigation, product edit, or commerce-write controls.
- Local asset editor asset type context diagnostics may expose read-only asset type rows from the selected local asset preview; they must remain diagnostics only and cannot become asset type mutation, task mutation, draft creation, publishing, sync, OAuth, credential, navigation, product edit, or commerce-write controls.
- Local asset editor review-state context diagnostics may expose read-only local asset review-state rows from the selected local asset preview; they must remain diagnostics only and cannot become product task status expansion, review mutation, draft creation, publishing, sync, OAuth, credential, navigation, product edit, or commerce-write controls.
- Local asset editor evidence summary context diagnostics may expose a read-only evidence count row derived from safe local claim previews; it must remain diagnostics only and cannot become claim editing, task mutation, draft creation, publishing, sync, OAuth, credential, navigation, product edit, or commerce-write controls.
- Local asset editor WordPress draft readiness context diagnostics may expose read-only blocked readiness, ready count, total count, and reconciliation markers for the selected local asset; they must remain diagnostics only and cannot become draft creation, page update, publishing, sync, OAuth, credential, navigation, product edit, or commerce-write controls.
- Local asset editor external-write clamp context diagnostics may expose read-only external-write false markers from the selected local asset preview; they must remain diagnostics only and cannot become external writes, draft creation, page update, publishing, sync, OAuth, credential, navigation, product edit, or commerce-write controls.
- Local asset editor commerce-write clamp context diagnostics may expose read-only commerce-write false markers from the selected local asset preview; they must remain diagnostics only and cannot become WooCommerce writes, product edits, price edits, inventory edits, external writes, draft creation, publishing, sync, OAuth, credential, navigation, or task mutation controls.
- Local asset editor credential-collection clamp diagnostics may expose read-only credential-collection false markers from the selected local asset preview; visible copy must avoid credential/OAuth collection wording and remain diagnostics only, without collecting or storing credentials, real OAuth, sync, external writes, draft creation, publishing, navigation, product edit, commerce-write, or task mutation controls.
- Local asset editor sync-execution clamp diagnostics may expose read-only sync-execution false markers from the selected local asset preview; visible copy must avoid sync/action wording and remain diagnostics only, without sync execution, real OAuth, credential collection/storage, external writes, draft creation, publishing, navigation, product edit, commerce-write, or task mutation controls.
- Local asset editor publishing clamp diagnostics may expose read-only publishing false markers from the selected local asset preview; visible copy must avoid publish/action wording and remain diagnostics only, without publishing, draft creation, page update, sync execution, real OAuth, credential collection/storage, external writes, product edit, commerce-write, navigation, or task mutation controls.
- Local asset editor WordPress page-update clamp diagnostics may expose read-only page-update false markers from the selected local asset preview; visible copy must remain diagnostics only, without WordPress page updates, publishing, draft creation, sync execution, real OAuth, credential collection/storage, external writes, product edit, commerce-write, navigation, or task mutation controls.
- Local asset editor WordPress draft-creation clamp diagnostics may expose read-only draft-creation false markers from the selected local asset preview; visible copy must remain diagnostics only, without WordPress draft creation, page update, publishing, sync execution, real OAuth, credential collection/storage, external writes, product edit, commerce-write, navigation, or task mutation controls.
- Local asset editor product-edit clamp diagnostics may expose read-only product-edit false markers from the selected local asset preview; visible copy must remain diagnostics only, without product edits, price edits, inventory edits, WooCommerce writes, draft creation, publishing, sync execution, real OAuth, credential collection/storage, external writes, navigation, or task mutation controls.
- Local asset editor price-edit clamp diagnostics may expose read-only price-edit false markers from the selected local asset preview; visible copy must remain diagnostics only, without price edits, inventory edits, product edits, WooCommerce writes, draft creation, publishing, sync execution, real OAuth, credential collection/storage, external writes, navigation, or task mutation controls.
- Local asset editor inventory-edit clamp diagnostics may expose read-only inventory-edit false markers from the selected local asset preview; visible copy must remain diagnostics only, without inventory edits, price edits, product edits, WooCommerce writes, draft creation, publishing, sync execution, real OAuth, credential collection/storage, external writes, navigation, or task mutation controls.
- Local asset editor stock-status edit clamp diagnostics may expose read-only stock-status edit false markers from the selected local asset preview; visible copy must remain diagnostics only, without stock status edits, inventory edits, price edits, product edits, WooCommerce writes, draft creation, publishing, sync execution, real OAuth, credential collection/storage, external writes, navigation, or task mutation controls.
- Local asset editor product-content edit clamp diagnostics may expose read-only product-content edit false markers from the selected local asset preview; visible copy must remain diagnostics only, without product content edits, stock status edits, inventory edits, price edits, product edits, WooCommerce writes, draft creation, publishing, sync execution, real OAuth, credential collection/storage, external writes, navigation, or task mutation controls.
- Local asset editor product-delete clamp diagnostics may expose read-only product-delete false markers from the selected local asset preview; visible copy must remain diagnostics only, without product deletion, product content edits, stock status edits, inventory edits, price edits, product edits, WooCommerce writes, draft creation, publishing, sync execution, real OAuth, credential collection/storage, external writes, navigation, or task mutation controls.
- Local asset editor product-image edit clamp diagnostics may expose read-only product-image edit false markers from the selected local asset preview; visible copy must remain diagnostics only, without product image edits, media uploads, product deletion, product content edits, stock status edits, inventory edits, price edits, product edits, WooCommerce writes, draft creation, publishing, sync execution, real OAuth, credential collection/storage, external writes, navigation, or task mutation controls.
- Local asset editor product-category edit clamp diagnostics may expose read-only product-category edit false markers from the selected local asset preview; visible copy must remain diagnostics only, without category edits, product image edits, media uploads, product deletion, product content edits, stock status edits, inventory edits, price edits, product edits, WooCommerce writes, draft creation, publishing, sync execution, real OAuth, credential collection/storage, external writes, navigation, or task mutation controls.
...
- Local asset editor blocked capability reconciliation diagnostics may expose a read-only marker proving visible blocked capability key rows match the editor blocked capability count; it must remain diagnostics only and cannot become claim editing, QA mutation, navigation, draft creation, publishing, sync, OAuth, credential, product edit, or commerce-write controls.
