...
```json
{
  ...
  "summary": {
    ...
    "by_automation_level": { "recommend_only": 1 },
...
- Frontend asset DTOs map `qa_checks` into safe local QA check previews with allowlisted keys (`seo`, `geo`, `factual_grounding`, `schema`, `local_review`) and statuses (`pending`, `passed`, `failed`). Unknown or unsafe values are clamped to `local_review` and `pending`, and raw QA metadata or credential-like values are not exposed to UI models.
