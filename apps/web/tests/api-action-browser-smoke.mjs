...
async function assertLocalAssetEditorWordPressDraftCreationClamp(editor, label) {
  const wordpressDraftCreationAllowed = await editor.getAttribute(
    "data-asset-editor-wordpress-draft-creation-allowed"
  );
  assert(
    wordpressDraftCreationAllowed === "false",
    `${label} editor WordPress draft-creation clamp mismatch: expected false, got ${
      wordpressDraftCreationAllowed ?? "missing"
    }`
  );
  const wordpressDraftCreationSummary = editor.locator(
    "[data-asset-editor-wordpress-draft-creation-summary='true']"
  );
  await expectVisible(wordpressDraftCreationSummary, `${label} visible editor WordPress draft-creation summary`);
  assert(
    (await wordpressDraftCreationSummary.getAttribute("data-asset-editor-wordpress-draft-creation-allowed")) ===
      "false",
    `${label} visible editor WordPress draft-creation summary clamp mismatch`
  );
  const summaryText = ((await wordpressDraftCreationSummary.textContent()) ?? "").toLowerCase();
  assert(
    summaryText.includes("false") || summaryText.includes("disabled"),
    `${label} visible editor WordPress draft-creation summary missing false/disabled copy`
  );
}

async function assertLocalAssetEditorProductEditClamp(editor, label) {
  const productEditAllowed = await editor.getAttribute("data-asset-editor-product-edit-allowed");
  assert(
    productEditAllowed === "false",
    `${label} editor product-edit clamp mismatch: expected false, got ${productEditAllowed ?? "missing"}`
  );
  const productEditSummary = editor.locator("[data-asset-editor-product-edit-summary='true']");
  await expectVisible(productEditSummary, `${label} visible editor product-edit summary`);
  assert(
    (await productEditSummary.getAttribute("data-asset-editor-product-edit-allowed")) === "false",
    `${label} visible editor product-edit summary clamp mismatch`
  );
  const summaryText = ((await productEditSummary.textContent()) ?? "").toLowerCase();
  assert(
    summaryText.includes("false") || summaryText.includes("disabled"),
    `${label} visible editor product-edit summary missing false/disabled copy`
  );
}

async function assertLocalAssetEditorPriceEditClamp(editor, label) {
  const priceEditAllowed = await editor.getAttribute("data-asset-editor-price-edit-allowed");
  assert(
    priceEditAllowed === "false",
    `${label} editor price-edit clamp mismatch: expected false, got ${priceEditAllowed ?? "missing"}`
  );
  const priceEditSummary = editor.locator("[data-asset-editor-price-edit-summary='true']");
  await expectVisible(priceEditSummary, `${label} visible editor price-edit summary`);
  assert(
    (await priceEditSummary.getAttribute("data-asset-editor-price-edit-allowed")) === "false",
    `${label} visible editor price-edit summary clamp mismatch`
  );
  const summaryText = ((await priceEditSummary.textContent()) ?? "").toLowerCase();
  assert(
    summaryText.includes("false") || summaryText.includes("disabled"),
    `${label} visible editor price-edit summary missing false/disabled copy`
  );
}
...
async function assertLocalAssetEditorStockStatusEditClamp(editor, label) {
  const stockStatusEditAllowed = await editor.getAttribute("data-asset-editor-stock-status-edit-allowed");
  assert(
    stockStatusEditAllowed === "false",
    `${label} editor stock-status edit clamp mismatch: expected false, got ${
      stockStatusEditAllowed ?? "missing"
    }`
  );
  const stockStatusEditSummary = editor.locator("[data-asset-editor-stock-status-edit-summary='true']");
  await expectVisible(stockStatusEditSummary, `${label} visible editor stock-status edit summary`);
  assert(
    (await stockStatusEditSummary.getAttribute("data-asset-editor-stock-status-edit-allowed")) === "false",
    `${label} visible editor stock-status edit summary clamp mismatch`
  );
  const summaryText = ((await stockStatusEditSummary.textContent()) ?? "").toLowerCase();
  assert(
    summaryText.includes("false") || summaryText.includes("disabled"),
    `${label} visible editor stock-status edit summary missing false/disabled copy`
  );
}

async function assertLocalAssetEditorProductContentEditClamp(editor, label) {
  const productContentEditAllowed = await editor.getAttribute("data-asset-editor-product-content-edit-allowed");
  assert(
    productContentEditAllowed === "false",
    `${label} editor product-content edit clamp mismatch: expected false, got ${
      productContentEditAllowed ?? "missing"
    }`
  );
  const productContentEditSummary = editor.locator("[data-asset-editor-product-content-edit-summary='true']");
  await expectVisible(productContentEditSummary, `${label} visible editor product-content edit summary`);
  assert(
    (await productContentEditSummary.getAttribute("data-asset-editor-product-content-edit-allowed")) === "false",
    `${label} visible editor product-content edit summary clamp mismatch`
  );
  const summaryText = ((await productContentEditSummary.textContent()) ?? "").toLowerCase();
  assert(
    summaryText.includes("false") || summaryText.includes("disabled"),
    `${label} visible editor product-content edit summary missing false/disabled copy`
  );
}

async function assertLocalAssetEditorProductDeleteClamp(editor, label) {
  const productDeleteAllowed = await editor.getAttribute("data-asset-editor-product-delete-allowed");
  assert(
    productDeleteAllowed === "false",
    `${label} editor product-delete clamp mismatch: expected false, got ${productDeleteAllowed ?? "missing"}`
  );
  const productDeleteSummary = editor.locator("[data-asset-editor-product-delete-summary='true']");
  await expectVisible(productDeleteSummary, `${label} visible editor product-delete summary`);
  assert(
    (await productDeleteSummary.getAttribute("data-asset-editor-product-delete-allowed")) === "false",
    `${label} visible editor product-delete summary clamp mismatch`
  );
  const summaryText = ((await productDeleteSummary.textContent()) ?? "").toLowerCase();
  assert(
    summaryText.includes("false") || summaryText.includes("disabled"),
    `${label} visible editor product-delete summary missing false/disabled copy`
  );
}
...
async function assertLocalAssetEditorProductCategoryEditClamp(editor, label) {
  const productCategoryEditAllowed = await editor.getAttribute("data-asset-editor-product-category-edit-allowed");
  assert(
    productCategoryEditAllowed === "false",
    `${label} editor product-category edit clamp mismatch: expected false, got ${
      productCategoryEditAllowed ?? "missing"
    }`
  );
  const productCategoryEditSummary = editor.locator("[data-asset-editor-product-category-edit-summary='true']");
  await expectVisible(productCategoryEditSummary, `${label} visible editor product-category edit summary`);
  assert(
    (await productCategoryEditSummary.getAttribute("data-asset-editor-product-category-edit-allowed")) === "false",
    `${label} visible editor product-category edit summary clamp mismatch`
  );
  const summaryText = ((await productCategoryEditSummary.textContent()) ?? "").toLowerCase();
  assert(
    summaryText.includes("false") || summaryText.includes("disabled"),
    `${label} visible editor product-category edit summary missing false/disabled copy`
  );
}
...
async function runSmoke() {
  ...
  try {
    ...
    await assertLocalAssetEditorProductCategoryEditClamp(populatedEditor, "populated asset workspace");
